...
function updateCsPicker() {
  let csPicker = colorsquarePicker;
 ...
 if (tracked) {
  ...
  switch (tracked.id) {
   case "colorsquare-event":
    xPercentage = (mouseX - trackedPos.x) / 150 * 100;
    yPercentage = (mouseY - trackedPos.y) / 90 * 100;
    ...
    break;
   case "hue-event":
    xPercentage = (mouseX - 10 - trackedPos.x) / 130 * 100;
    (xPercentage > 100) ? xPercentage = 100: (xPercentage < 0) ? xPercentage = 0 : null;
    xPercentage = xPercentage.toFixed(2);
    picker = huePicker;
    picker.style.left = xPercentage + "%";
...
function csDown(event) {
 ...
 let xPercentage = ((event.offsetX + 1) / 150) * 100;
 let yPercentage = ((event.offsetY + 1) / 160) * 100;
 yPercentage = yPercentage.toFixed(2);
 xPercentage = xPercentage.toFixed(2)
 let picker = colorsquarePicker;
 picker.style.top = yPercentage + "%";
 picker.style.left = xPercentage + "%";
 s = xPercentage / 100;
 v = 1 - yPercentage / 100;
...
function hueDown(event) {
 tracked = event.currentTarget;
 let xPercentage = ((event.offsetX - 9) / 130) * 100;
 xPercentage = xPercentage.toFixed(2);
...
function hueDownTouch(event) {
 ...
 let rect = event.target.getBoundingClientRect();
 let offsetX = event.targetTouches[0].clientX - rect.left;
 let xPercentage = ((offsetX - 9) / 130) * 100;
 xPercentage = xPercentage.toFixed(2);
 let picker = huePicker;
 picker.style.left = xPercentage + "%";
 h = xPercentage / 100;
...
function hueChange() {
 ...
 let colors = colorsquare;
 colors.style.background = `rgba(${rgb[0]},${rgb[1]},${rgb[2]},1)`;
 colorChange();
}
...
<div class="main-container" style={`width: ${width}px; height: ${height}px`}>
  ...
  <div class="colorsquare size" bind:this={colorsquare}>
      <div class="saturation-gradient">
          <div class="value-gradient">
              <div id="colorsquare-picker" bind:this={colorsquarePicker}></div>
              <div id="colorsquare-event" bind:this={colorsquareEvent} on:mousedown={csDown} on:touchstart={csDownTouch}></div>
          </div>
      </div>
  </div>
  ...
  <div class="hue-selector">
      ...
      <div id="hue-event" bind:this={hueEvent} on:mousedown={hueDown} on:touchstart={hueDownTouch}></div>
  </div>

  
  <div class="alpha-selector">
      <div class="alpha-value"></div>
      <div id="alpha-picker" bind:this={alphaPicker}></div>
      <div id="alpha-event" bind:this={alphaEvent} on:mousedown={alphaDown} on:touchstart={alphaDownTouch}></div>
  </div>
